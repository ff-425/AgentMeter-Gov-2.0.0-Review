"""Run a V5 partition through the installed, real local OpenClaw gateway.
...
DEFAULT_BACKEND_HEALTH_URL = "http://127.0.0.1:8765/health"
...
def observed_action(events: list[dict[str, Any]], final_text: str, session_raw: str) -> tuple[str, bool, list[int]]:
    ...
    if re.search(r"CTRL-BLOCK|AgentMeter-Gov.{0,40}(?:阻断|blocked)|已阻断|guard blocks", combined, re.I | re.S):
...
def main() -> int:
    ...
    fixture_root = workspace / "agentmeter_v5_live" / f"{args.partition}-{run_stamp}"
    ...
    try:
        for index, case in enumerate(rows, 1):
            ...
            session_key = f"agent:main:agentmeter-v5-{args.partition}-{run_stamp}-{case_id.lower()}"
    ...
    report = {
        "schema_version": "agentmeter.gov-v5-live-results.v1",
